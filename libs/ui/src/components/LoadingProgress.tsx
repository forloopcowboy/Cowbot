...
export default function LoadingProgress({
  ...
  // Each SSE event may now carry multiple newline-separated lines (the API
  // server batches them on a short debounce). Split before parsing so the
  // banner/info/done detection still operates per logical line.
...
function FriendlyView({
  ...
  return (
    <div className="text-sm">
      ...
      {showQuip && (
        <div className="mt-4 flex items-center gap-2 text-slate-500 dark:text-slate-400 italic">
          <Spinner />
      ...
      {stage === 'done' && (
        <div className="mt-4 flex items-center gap-2 text-emerald-700 dark:text-emerald-300">
          <span aria-hidden>✓</span>
          <span>All done.</span>
        </div>
      )}
      {stage === 'error' && (
        <div className="mt-4 flex items-center gap-2 text-red-700 dark:text-red-300">
          <span aria-hidden>✗</span>
...
function SectionPanel({
  ...
  return (
    <div className="rounded border border-slate-200 bg-white dark:border-navy-700 dark:bg-navy-900">
      <DisclosureButton className="w-full flex items-center gap-2 px-3 py-2 hover:bg-slate-50 dark:hover:bg-navy-800/60 focus:outline-none focus:ring-1 focus:ring-gold-500 rounded">
        <Chevron open={open} />
        <span className="text-[10px] uppercase tracking-[0.2em] text-gold-600 dark:text-gold-300 font-medium">
          {banner}
        </span>
        <span className="flex-1 border-t border-slate-200 dark:border-navy-800 mx-1" />
        <span className="flex items-center gap-2 text-[10px] text-slate-500 dark:text-slate-400">
          ...
          {warnCount > 0 && <span className="text-amber-700 dark:text-amber-300">{warnCount} warn</span>}
          {errCount > 0 && <span className="text-red-700 dark:text-red-300">{errCount} err</span>}
        </span>
      </DisclosureButton>
      <DisclosurePanel className="px-3 pb-2">
        <ul className="space-y-1">
          ...
          {section.events.length === 0 && (
            <li className="text-slate-400 dark:text-slate-500 italic">no entries yet</li>
          )}
        </ul>
      </DisclosurePanel>
    </div>
...
function EventIcon({ kind }: { kind: Event['kind'] }) {
  if (kind === 'done')
    return (
      <span className="mt-[3px] text-emerald-600 dark:text-emerald-300" aria-hidden>
        ✓
      </span>
    )
  if (kind === 'warn')
    return (
      <span className="mt-[3px] text-amber-600 dark:text-amber-300" aria-hidden>
        !
      </span>
    )
  if (kind === 'error')
    return (
      <span className="mt-[3px] text-red-600 dark:text-red-300" aria-hidden>
        ✗
      </span>
    )
  // info / fallback
  return (
    <span className="mt-[3px] text-slate-400 dark:text-slate-500" aria-hidden>
      ·
    </span>
  )
}
          ...
          <span key={s.id} className="flex items-center gap-1.5">
            <span
              className={[
                ...
                state === 'done' && 'bg-navy-50 text-navy-700 dark:bg-navy-800 dark:text-gold-300',
                state === 'current' && 'bg-gold-500 text-white dark:text-ink',
                state === 'error' && 'bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-300',
                state === 'pending' && 'bg-slate-100 text-slate-500 dark:bg-navy-800/60 dark:text-slate-400',
              ]
                .filter(Boolean)
                .join(' ')}
            >
            ...
            {i < STAGES.length - 1 && <span className="text-slate-400 dark:text-slate-500">→</span>}
          </span>
        )
      })}
    </div>
