...
export default function ProfileWizard({ to }: { to?: string }) {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      navigate(`/app/p/${state.name.trim()}/holdings`)
    } catch (e) {
  ...
  return (
    <div className="min-h-full bg-canvas dark:bg-ink flex flex-col">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-2 pb-16 w-full">
        ...
        <h1 className="font-serif text-2xl sm:text-3xl font-semibold text-navy-900 dark:text-cream">
          {t('wizard.title')}
        </h1>
        <p className="mt-2 text-slate-600 dark:text-slate-300 text-sm">{t('wizard.subtitle')}</p>
        ...
        {error && (
          <div className="mt-3 px-3 py-2 rounded border border-red-200 bg-red-50 text-sm text-red-700 dark:border-red-500/40 dark:bg-red-500/10 dark:text-red-300">
            {error}
          </div>
        )}
...
function Stepper({ step, t }: { step: number; t: TFunction }) {
  return (
    <ol className="mt-6 sm:mt-8 flex flex-wrap gap-x-3 sm:gap-x-6 gap-y-2 text-xs">
      {STEP_KEYS.map((key, i) => {
        const n = i + 1
        const active = n === step
        const done = n < step
        return (
          <li
            ...
            className={`flex items-center gap-2 ${
              active
                ? 'text-navy-900 dark:text-cream font-semibold'
                : done
            ...
            <span
              className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] ${
                active
                  ...
                  : done
                    ? 'bg-gold-500 text-white dark:text-ink'
                    : 'bg-slate-200 text-slate-500 dark:bg-navy-800 dark:text-slate-400'
              }`}
            >
              {n}
            </span>
          ...
          {RISK_OPTIONS.map((r) => (
            <button
              ...
              className={`text-left rounded border px-3 py-2 ${
                state.risk === r
                  ? 'border-navy-800 bg-navy-50 dark:border-gold-500 dark:bg-navy-800'
                  : 'border-slate-200 bg-white hover:border-slate-300 dark:border-navy-700 dark:bg-navy-900 dark:hover:border-navy-600'
              }`}
            >
              <div className="font-semibold text-sm capitalize text-navy-900 dark:text-cream">{r}</div>
              <div className="text-xs text-slate-500 dark:text-slate-400">{t(`wizard.step2.riskDesc.${r}`)}</div>
            </button>
          ))}
        </div>
      </Field>
...
function Step5({ state, set, errors, t }: StepProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {errors.accounts && (
        <div className="text-xs text-red-600 dark:text-red-400">{errors.accounts}</div>
      )}
    ...
    <div className="space-y-5">
      ...
      <Field label={t('wizard.step6.etfWatchlist')}>
        <TickerSearcher
          ...
          suggested={COMMON_ETFS.map((e) => e.ticker)}
        />
      </Field>
...
function Step7({ state, set, t }: { state: WizardState; set: SetFn; t: TFunction }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {isBelgium && (
        <div className="rounded border border-slate-200 bg-slate-50/60 p-3 text-xs text-slate-700 dark:border-navy-700 dark:bg-navy-800/40 dark:text-slate-300">
          <div className="font-semibold text-navy-900 mb-1 dark:text-cream">{t('wizard.step7.belgianBlockTitle')}</div>
      ...
      <details className="rounded border border-slate-200 p-3 dark:border-navy-700">
        <summary className="cursor-pointer text-xs font-semibold text-navy-900 dark:text-cream">
...
function SectionHeader({ title, hint }: { title: string; hint?: string }) {
  return (
    <div>
      <h2 className="font-serif text-lg text-navy-900 dark:text-cream font-semibold">{title}</h2>
    ...
    <div>
      <div className="flex flex-wrap gap-1.5">
        {values.map((v) => (
          <span
            key={v}
            className="inline-flex items-center gap-1 rounded bg-navy-50 border border-navy-100 px-2 py-0.5 text-xs text-navy-800 dark:bg-navy-800 dark:border-navy-700 dark:text-cream"
          >
            {v}
            <button
              type="button"
              onClick={() => remove(v)}
              className="text-navy-500 hover:text-red-600 dark:text-slate-400 dark:hover:text-red-400"
        ...
        {values.length === 0 && (
          <span className="text-xs text-slate-400 dark:text-slate-500">{t('common.none')}</span>
        )}
      </div>
      {suggested.length > 0 && (
