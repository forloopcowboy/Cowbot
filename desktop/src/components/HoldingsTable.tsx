      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-navy-50 border-b border-slate-200">
              {cols.map((c) => (
                <th
                  key={c.key as string}
                  className={`text-left px-2.5 py-2 text-xs font-medium uppercase tracking-wide text-navy-700 ${c.w ?? ''}`}
                >
                  {t(c.labelKey)}
                </th>
              ))}
          ...
          <tbody>
              ...
              <tr key={i} className="border-b border-slate-100 hover:bg-slate-50">
                {cols.map((c) => (
                  <td key={c.key as string} className="px-1.5 py-1">
                    <input
                      ...
                      className="w-full !border-transparent hover:!border-slate-200 focus:!border-navy-700 !py-1 !px-1.5"
                    />
                  </td>
                ))}
                <td className="px-1 py-1">
                  <button
                    type="button"
                    onClick={() => remove(i)}
                    className="text-slate-400 hover:text-red-600 px-1.5 py-1 text-sm"
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={cols.length + 1} className="text-center text-sm text-slate-500 py-6">
